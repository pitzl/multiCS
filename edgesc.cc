...
int main()
{
  ...
  double tz = 0.5; // [cm]
  cout << "  THICKNESS z ..................... " << tz*10 << " mm" << endl;

  double ty = 0.0150; // [cm]
  cout << "  THICKNESS y ..................... " << ty*10 << " mm" << endl;
  ...
  double eloss = 0.3 * tz*1e4;     // MIP [keV]
  ...
  double chic2 = 0.157 * Z*(Z+1) * tz*rho / A / e0mev / e0mev;
  ...
  cout << "Lynch-Dahl RMS scattering " << rmstet*1e3 << " mrad" << endl;
  cout << endl;

  double dz = tz / a0b;  // atomic units
  double dy = ty / a0b;  // atomic units
  ...
  TFile * histoFile = new TFile( "edgesc.root", "RECREATE" );

  // book histos:

  TH1I hde( "de", "energy loss;energy loss [keV];tracks", 200, 0, 10*eloss );
  TH1I htet( "tet", "exit angle;exit angle [mrad];tracks", 200, 0, 10*rmstet*1e3 );
  TH1I hy( "y", "exit y;exit y [#mum];tracks", 250, -50, ty*1e4+50 );
  TH1I hx( "x", "exit x;exit x [#mum];tracks", 250, -50, ty*1e4+50 );

  TH2I * h2xy = new
    TH2I( "xy",";x [#mum];y [#mum]",
	  150, 0, ty*1e4, 150, 0, ty*1e4 );
  ...
  while( ntot < ntotal ) { // tracks
    ...
    double x = dy*unirnd(rgen);
    double y = dy*unirnd(rgen);
    double z = 0;
    ...
    if( iexit == 0 ) { // transmitted
      ...
      hx.Fill( x*a0b*1e4 ); // smeared
      hy.Fill( y*a0b*1e4 ); // truncated
      h2xy->Fill( x*a0b*1e4, y*a0b*1e4 );

      EM += E; // exit energy [atomic]
      VEM += E*E;

      hde.Fill( e0kev - E*HRKEV );
  ...
  cout << "top out        " << N[2] << endl;
  cout << "bot out        " << N[3] << endl;
  cout << "absorbed       " << N[4] << endl;
